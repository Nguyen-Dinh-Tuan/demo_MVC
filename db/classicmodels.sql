...
--
-- Host: localhost
-- Generation Time: Dec 02, 2020 at 10:52 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `findAllCustomers` ()  BEGIN

   SELECT *  FROM customers where customerNumber = 175;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getCusById` (IN `cusNUM` INT(11))  BEGIN 
SELECT * FROM customers WHERE customerNumber = cusNum;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCustomersCountByCity` (IN `in_city` VARCHAR(50), OUT `total` INT)  BEGIN
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `CUSTOMERS_VIEW`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName` FROM `customers` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_views`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName`, `customers`.`contactFirstName` AS `contactFirstName`, `customers`.`contactLastName` AS `contactLastName`, `customers`.`phone` AS `phone` FROM `customers` WHERE (`customers`.`city` = 'Nantes') ;
...
ALTER TABLE `customers`
  ...
  ADD UNIQUE KEY `index_Name` (`customerName`),
